...
object ChatCommands : Module(
    ...
    private val coopChatCommands by BooleanSetting("Co-op Commands", false, "Enables coop chat commands.")
    ...
    init {
        ...
        on<MessageSentEvent> {
            if (!chatEmotes || (message.startsWith("/") && !listOf("/pc", "/ac", "/gc", "/msg", "/w", "/r", "/cc", "/cchat").any { message.startsWith(it) })) return@on
